...
def check_cols(table, called_nums):
    # global bingo
    matrix = np.matrix(table)
    for col in matrix.T:
        check = [k for k in col.A1 if k in called_nums]
        if len(check)==5: 
            bingo = True
            return check
        # else:
...
def check_tables(tables, called_nums):
    for table in tables:
        row_check = check_rows(table, called_nums)
        col_check =  check_cols(table, called_nums)
        if bingo == True:
            return (row_check,col_check,table)
    return (row_check,col_check,table)

# --- PART 1 ANSWER ---
bingo = False
chosen_nums = []
for number in all_bingo_nums:
    chosen_nums.append(number)
    result = check_tables(tables,chosen_nums)
    # print(result, bingo)
    if bingo: print(result, bingo, number) ; break

ans1 = chosen_nums[-1] * (np.array([k for k in np.matrix(result[2]).A1 if k not in chosen_nums]).sum())
ans1 # 67716


# --- PART 2 FUNCTIONS ---
def check_table(matrix,called_nums):
    all_matches = []
    for col in matrix.T:
        col_check = all_matches.append([k for k in col.A1 if k in called_nums])
    for row in matrix:
        row_check = all_matches.append([k for k in row.A1 if k in called_nums])
    bingo_arrays = [k for k in (all_matches) if len(k)==5]
    return bingo_arrays

check_table(np.matrix(tables[0]), all_bingo_nums[:53])
...
for number in all_bingo_nums:
    ...
    for ind, table in enumerate(tables):
        check = check_table(np.matrix(table),chosen_nums)
        if len(check) > 0:
            bingo_tables.append(table)
    if len(bingo_tables) == 99:
        break

last_table = [k for k in tables if k not in bingo_tables][0]

for number in all_bingo_nums:
    bingo_tables = []
    chosen_nums.append(number)
    check = check_table(np.matrix(last_table),chosen_nums)
    if len(check) > 1:
        break

ans2 = chosen_nums[-1] * (np.array([k for k in np.matrix(last_table).A1 if k not in chosen_nums]).sum())
ans2 # 1830
